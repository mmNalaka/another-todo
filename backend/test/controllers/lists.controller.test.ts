...
import * as listsRepo from '@/db/repositories/lists.repo'
...
describe('lists Controller', () => {
  ...
  beforeEach(() => {
    ...
    mockContext = {
      ...
      req: {
        valid: vi.fn((type) => {
          ...
          if (type === 'json')
            return { title: 'Updated List', description: 'Updated Description' }
    ...
    vi.mocked(responseUtils.createSuccessResponse).mockReturnValue(
      'success-response' as any,
    )
    vi.mocked(responseUtils.createPaginatedResponse).mockReturnValue(
      'paginated-response' as any,
    )
    vi.mocked(responseUtils.calculatePagination).mockReturnValue({
      page: 1,
      limit: 10,
      total: 2,
      pages: 1,
    ...
    vi.mocked(errorUtils.createErrorResponse).mockReturnValue(
  ...
  describe('getAllListHandler', () => {
    it('should return all personal lists for a user', async () => {
      ...
      const handlerFn = listsController.getAllListHandler[
        listsController.getAllListHandler.length - 1
      ] as any
  ...
  describe('createListHandler', () => {
    it('should create a new list', async () => {
      // Setup mock repository response
      vi.mocked(listsRepo.createPersonalList).mockResolvedValue([
        mockList,
      ] as any)
      ...
      mockContext.req.valid = vi.fn((type) => {
        if (type === 'json')
          return { title: 'New List', description: 'New Description' }
        return {}
      })

      // Extract the handler function
      const handlerFn = listsController.createListHandler[
        listsController.createListHandler.length - 1
      ] as any
  ...
  describe('getListByIdHandler', () => {
    it('should return a list by id when user is owner', async () => {
      ...
      vi.mocked(listsRepo.getListCollaborators).mockResolvedValue(
        mockCollaborators,
      )
      vi.mocked(listsRepo.getListTasks).mockResolvedValue(mockTasks)

      // Extract the handler function
      const handlerFn = listsController.getListByIdHandler[
        listsController.getListByIdHandler.length - 1
      ] as any
  ...
  describe('updateListHandler', () => {
    it('should update a list when user is owner', async () => {
      ...
      // Extract the handler function
      const handlerFn = listsController.updateListHandler[
        listsController.updateListHandler.length - 1
      ] as any
  ...
  describe('deleteListHandler', () => {
    it('should delete a list when user is owner', async () => {
      ...
      // Extract the handler function
      const handlerFn = listsController.deleteListHandler[
        listsController.deleteListHandler.length - 1
      ] as any
  ...
  describe('toggleListFrozenHandler', () => {
    it('should toggle frozen state when user is owner', async () => {
      ...
      // Extract the handler function
      const handlerFn = listsController.toggleListFrozenHandler[
        listsController.toggleListFrozenHandler.length - 1
      ] as any
